...
s2l (Scons e1 e2) = 
    let e1' = s2l e1
        e2' = s2l e2
    in case (e1', e2') of
	 (Lvar "lambda", Lvar v) -> Llambda [v] (Lvar " ") -- Commence par prendre la variable
	 (Lvar "cons", Lvar n) -> Lcons n []  -- Crée un tableau vide
	 (Lvar "case", _ ) -> Lcase e2' []  -- Commence par prendre l'expression "évalué"
	 --(Lcase e tab, _)
	 (Lcons tag tab, _ ) -> Lcons tag (tab++[e2'])  -- Prendre le tableau crée et on lui ajoute les expression suivante WARNING elle prendra toute les autres
	 ( _ , Lvar v ) -> Lapp e1' [e2']  -- Surtout pour les fonction Lambda
	 ( _ , Lnum n) -> Lapp e1' [e2'] -- Pour donner les entiers
	 ((Llambda var exp), _ ) -> Llambda var e2'  -- Associe l'expresion à la variable 
